...
\begin{jtheorem}[アルゴリズム的同値の宣言的同値に関する健全性]\label{SoundAQ}
...
\begin{theorem}[アルゴリズム的型付け及びカインド付けの健全性]\label{SoundAKT}
    任意の ${n \in \mathbb{N}}$,
    \begin{itemize}
...
\begin{jtheorem}[アルゴリズム的型付けの完全性]\label{AlgorithmicComp}
    ...
    \begin{proof}
        \begin{itemize}
            ...
            ${\Gamma \myvdash{n} S \DCOL \Pi x \COL T_1. K}$ に帰納法の仮定を適用することにより,
            ${\Gamma \myalvdash{n} J \myequiv{m} \Pi x \COL T_1 . K} \cdots (1)$ かつ
            ${\Gamma \myalvdash{n} S \DCOL J \cdots (2)}$ であるような $J$ が存在することがわかる.
            (1) の導出は \fauxsc{QKA-Pi} によるので, $J \equiv \Pi x \COL T_1. K'$,
            ${\Gamma \myalvdash{n} T_1 \myequiv{m} T_2 \cdots (3)}$ かつ ${\Gamma x \mysncln{n} T_1 \myalvdash{n} K_1 \myequiv{m} K_2 \cdots (4)}$ であるとわかる.
            また, ${\Gamma \myvdash{n} T_1 \DCOL K_1}$ に帰納法の仮定を適用することにより,
            ${\Gamma \myalvdash{n} T_1 \DCOL K_1} \cdots (5)$ を得る.
            最後に, \fauxsc{QK-App} と (3), (4), (5) を用いることで,
            ${\Gamma \myalvdash{n} S t \DCOL [x \mapsto t] K_1}$ を得る.
