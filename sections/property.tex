...
\begin{jlemma}[代入補題]\label{Substitution}
    ...
    \begin{proof}
        ...
        \begin{inditem}
            ...
            \begin{enumerate}
                \item{y = x}\\
                $\Gamma, x \mysncln{n_1} \myvdash{n_2} x \COL T$ and $\Gamma \myvdash{n_1} s \COL S$ とする.
                環境内に同じバインドを持つ変数は存在し得ないことから, 
                $\Gamma$ は, $x$ への束縛を有せず, $n_1 = n_2$ とわかる.
                また, $S$ は自由変数 $x$ を含まないため, $[x \mapsto s] S = S$ である.
                よって, $\Gamma \myvdash{n_1} s \COL S$ を示せば十分であり, 示された.
                \item{$y \neq x$}\\
                $\Gamma, x \mysncln{n_1} S \myvdash{n_2} y \COL T$ and $\Gamma \myvdash{n_1} s \COL S$ とする.
            ...
            かつ $\Gamma \myvdash{n_1} s \COL S$, であるなら, $\Gamma \myvdash{n_2} \mylam{y}{S'}{[x \mapsto s] t'} \COL \Pi y \COL S'. [x \mapsto s] T'$.
            \\$\Gamma \myvdash{n_2} s \COL S$, のとき, weakening より, $\Gamma, y \mysncln{n_2} S' \myvdash{n_1} s \COL S$.
            よって, 帰納法の仮定より, \(\Gamma, y \mysncln{n_2} S' \myvdash{n_2} [x \mapsto s] t' \COL [x \mapsto s] T'\).
            \\\fauxsc{T-Abs} によって, 示したい命題が得られる.
            \begin{prooftree}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        帰納法の仮定より,  
            $\Gamma \myvdash{n-1} s \COL \bigcirc T$ かつ $\myval{t}{d} \iarrow{i} \myval{t'}{d}$ であるならば, $\Gamma \myvdash{n-1} s'\COL T$.
        \begin{enumerate}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                値の定義より, $t_1 \in v_{i+1}$ であるなら,  $\mynext{t_i} \in v_{i}$.
            \end{itemize}
        \end{itemize}
    \end{proof}
\end{jtheorem}
...
\begin{jtheorem}[強正規化性]\label{SN}
    ...
    \begin{proof}
        ...
        のような無限長の簡約列が存在する. 
        \Cref{FiniteStageReduction}より, この簡約列には無限個の \mbeta-簡約 あるいは 同値型に関する簡約ステップが存在から,
        よって, \Cref{BetaTransPreserve} と \Cref{EqualityTransPreserve} より, \lamlf でも $\strans{t}$ の無限長の簡約列が存在する.
        これは, \lamlf が強正規化性を持つことに矛盾している.
    \end{proof}
\end{jtheorem}
\vspace{10pt}
...
\begin{jlemma}[並行簡約のダイヤモンド性]\label{DiamondParallel}
    任意の自然数 $i$ と任意の項 $s$ について, $s \parrow{i} t_1$ かつ $s \parrow{i} t_2$ であるなら, 
    $t_1 \parrow{i} t$ and $t_2 \parrow{i} t$ が成立する.
    \begin{proof}
        はじめに, 項 $M$ に対して, 項 $\tilde{M}$ を定義する.
        \begin{itemize}
            \item $M \equiv x$ であるなら, $\tilde{M} \equiv x$
            \item $M = \mylam{x}{T}{M_1}$ であるなら $\tilde{M} = \mylam{x}{T}{\tilde{M_1}}$
            \item $M = M_1 M_2$ かつ $M$ \mbeta-基でないなら, $\tilde{M} = \tilde{M_1} \tilde{M_2}$
            \item $M = (\mylam{x}{T}{M_1}) M_2$ であるなら, $\tilde{M} = [x \mapsto \tilde{M_2}] \tilde{M_1}$
            \item $M = \mynext{M_1}$ かつ $M$ がステージ簡約基でないなら, $\tilde{M} = \mynext{\tilde{M_1}}$ 
            \item $M = \mynext{\myprev{M_1}}$ であるなら, $\tilde{M} = \tilde{M_1}$ 
            \item $M = \myprev{M_1}$ かつ $M$ がステージ簡約基でないなら $\tilde{M} = \myprev{\tilde{M_1}}$ 
            \item $M = \myprev{\mynext{M_1}}$ であるなら, $\tilde{M} = \tilde{M_1}$
            % \item If $M = \myidpeel{M_1}{x}{M_2}$ and $M$ is not a stage redex, $\tilde{M} = M$
            \item $M = \myidpeel{\myid{M_1}}{x}{M_2}$ であるなら, $\tilde{M} = [x \mapsto M_1] \tilde{M_2}$
        \end{itemize}
        $M \parrow{i} N$ であるなら, $N \parrow{i} \tilde{M}$ であることを示す. これは, 並行簡約のダイヤモンド性を示している.\\
        \textbf{$M$ の構造に関する帰納法.}
        \begin{itemize}
            \item{$M = x$}\\
            Immediate.
           ...
           これより, $N_1 N_2 \parrow{i} \tilde{M_1} \tilde{M_2}$ が示される.
            ...
            \begin{itemize}
                ...
                \item{$N = N_0 N_2$}\\
                帰納法の仮定より, ${N_0 \parrow{i} \mylam{x}{T}{\tilde{M_1}}}$ and ${N_2 \parrow{i} \tilde{M_2}}$ が成り立つ.
...
\begin{jtheorem}[合流性]\label{ChurchRosser}
    ...
    $r \rightarrow^* t$ and $s \rightarrow^* t$ であるような項 $t$ が存在する.
    \begin{proof}
